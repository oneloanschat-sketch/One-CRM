...
import { X, ArrowRight, FileText, Phone, Calendar, Info } from 'lucide-react';
  ...
  const getDynamicValue = (client: Client) => {
    switch (type) {
      ...
      case 'WAIT_TIME':
        ...
        return (
            <span className={`font-bold text-sm ${isCritical ? 'text-red-600' : 'text-slate-600'}`}>
      ...
      <div className="relative bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="p-4 md:p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div>
            ...
            <p className="text-slate-500 text-xs md:text-sm mt-1">נמצאו {clients.length} לקוחות רלוונטיים</p>
          </div>
          <button 
            onClick={onClose}
        ...
        {/* List */}
        <div className="overflow-y-auto p-4 custom-scrollbar bg-slate-50/30">
          {clients.length === 0 ? (
